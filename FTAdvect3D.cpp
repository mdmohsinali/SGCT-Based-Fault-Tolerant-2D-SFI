...
void FTAdvect3D::initGridApp(bool useFullGrid, FTSparseGrid3D * uvsg, HaloArray3D *u, double * doubleAppFi, int appFiSize, 
                  MPI_Comm comm, int spec, int specIndex, bool isComponentGrid,
  bool is2D){
  int rank;
  MPI_Comm_rank(comm, &rank);
  int doubleAppFiSize, doubleFiSizeWithoutSpec, startDoubleFiIndex, endDoubleFiIndex;
  ...
  // Count of double type elements in App's field array fi
  doubleAppFiSize = appFiSize;

  if (isComponentGrid) { // component grid
     doubleFiSizeWithoutSpec = doubleAppFiSize/spec;
     ...
     endDoubleFiIndex = doubleAppFiSize - 1;
  }
  ...
  if (is2D) { // 2D
     ...
     for(int i = startDoubleFiIndex; i <= endDoubleFiIndex; i++){
        ...
        Vh(u, adv2DIndexX, adv2DIndexY, 0) = doubleAppFi[i];       
     }
  }
  else { // 3D
     #pragma omp parallel for default(shared)
     for(int i = startDoubleFiIndex; i <= endDoubleFiIndex; i++){
        ...
        Vh(u, adv3DIndexX, adv3DIndexY, adv3DIndexZ) = doubleAppFi[i];     
     }
  }
...
#pragma omp parallel for default(shared)        
  ...
  */
} //initGridApp (with HaloArray)

void FTAdvect3D::initGridApp(bool useFullGrid, FTSparseGrid3D * uvsg, FTHaloArray3D *u, double * doubleAppFi, int appFiSize, 
                  MPI_Comm comm, int spec, int specIndex, bool isComponentGrid,
  bool is2D){
  int rank;
  MPI_Comm_rank(comm, &rank);
...
#pragma omp parallel for default(shared)        
  ...
  */
} //initGridApp (with FTHaloArray)


double FTAdvect3D::checkErrorApp(HaloArray3D *u, FTHaloArray3D *v, bool is2D, bool isReadFromDisk) {
  double err = 0.0;
  ...
  return (err);
} //checkErrorApp (interpolation on full grid)

double FTAdvect3D::checkErrorApp(FTSparseGrid3D *u, FTHaloArray3D *v, bool is2D, bool isReadFromDisk) {
  double err = 0.0;

  if (u->useFullGrid) {
     return (checkErrorApp(u->uh, v, is2D, isReadFromDisk)); 
  }
...
#pragma omp parallel for default(shared) reduction(+:err)
  ...
  return (err);
} //checkErrorApp (interpolation on sparse grid)

...
static double * AppFiSG = NULL;  // array (1D double) to cache App's sparse grid field
...
void callAppSubroutines(ProcGrid3D *g, int gridSizeX, int gridSizeY, int gridSizeZ,
        ...
        */
        ...
        if (isRealCall) {
           c_runApp(parInDir, &procGridX, &procGridY, &procGridZ, &fortranCommWorld, 
...
void visualizeSparseGrid (void) {
        ...
        for (int k = 0; k < lz; k++) {
           for (int j = 0; j < ly; j++) {
              for (int i = 0; i < lx; i++) {
                 AppFiSG[sgIndex] = Vh(uvsg->uh, i, j, k);
                 sgIndex++;
              }
           }
        }
        ...
        visualizeField(&(sg->P.x), &(sg->P.y), &(sg->P.z), &fortranCommWorld,
                 &gridSizeX, &gridSizeY, &gridSizeZ, &fieldSize, AppFiSG);

        delete [] AppFiSG;

        // Call python script for visualization
        if (sg->myrank == 0) {
           system("python ../../src/testing/rho_visualize_lbm.py");
        }
} //visualizeSparseGrid()  

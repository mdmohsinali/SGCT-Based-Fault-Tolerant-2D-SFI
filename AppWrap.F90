...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!
SUBROUTINE c_runApp(in_dir, proc_x, proc_y, proc_z, f_comm, comp_g, rep_comb, n, &
                    nx, ny, nz, n_comb, real_call) BIND(C, NAME='c_runApp')
       ...
       INTERFACE
          SUBROUTINE c_get_app_field(dimen, f_1d, f_2d, f_3d, xs, ys, zs, xm, ym, zm, comp_g, rep_comb) BIND (C, NAME = 'c_get_app_field')
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      call MPI_Comm_rank(f_comm,user%rank,ierr)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      call SNESCreate(f_comm,snes,ierr)
...
! stencil temporarily.
     ...
     !&     ione,ione,PETSC_NULL_INTEGER,PETSC_NULL_INTEGER,da,ierr)
      call DMDACreate2d(f_comm,                               &
     &     DM_BOUNDARY_NONE, DM_BOUNDARY_NONE,                          &
     &     DMDA_STENCIL_BOX,nx,ny,proc_x,proc_y,          &
     &     ione,ione,PETSC_NULL_INTEGER,PETSC_NULL_INTEGER,da,ierr)
...
!Mohsin added this
      !call VecView(x,PETSC_VIEWER_STDOUT_WORLD, ierr)
      call DMDAVecGetArrayF90(da, x, x2,ierr)
      do j1 = user%ys-1, user%ye-1
          do i1 = user%xs-1, user%xe-1
              !write(*,*) 'rank = ', user%rank, ' val = ', x2(i1, j1) !write(*,'("val = ", g8.6)') x2(i1, j1)
          enddo
      enddo
      call DMDAVecRestoreArrayF90(da, x, x2, ierr)

      my_dims = 2
      zs = 0
      zm = 1
      if(real_call) then
         call c_get_app_field(my_dims, x1, x2, x3, user%xs-1, user%ys-1, zs, &
                              user%xm, user%ym, zm, comp_g, rep_comb)
      end if
